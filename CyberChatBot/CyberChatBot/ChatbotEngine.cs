...
namespace CyberChatBot
{
    ...
    public class ChatbotEngine
    {
        ...
        private readonly NlpHelper _nlp = new NlpHelper();

        /// <summary>True while a quiz is in progress, so MainWindow knows to route input to the quiz panel instead of normal chat.</summary>
        public bool IsQuizActive { get; private set; }

        /// <summary>Exposes the activity log so the UI can display it directly if needed.</summary>
        public ActivityLog Activity => _activityLog;

        /// <summary>Exposes the task manager so the UI can render task lists/buttons.</summary>
        public TaskManager Tasks => _taskManager;

        /// <summary>Exposes the quiz engine so the UI can render the quiz panel.</summary>
        public QuizEngine Quiz => _quizEngine;
            ...
            ["social engineering"] = new List<string>
    {
        ...
        // ─────────────────────────────────────────────
        //  Part 3 — Feature Intent Handling
        // ─────────────────────────────────────────────
        ...
        {
            NlpResult nlpResult = _nlp.Interpret(rawInput);
            ...
            switch (nlpResult.Intent)
            {
                ...
                case Intent.SetReminder:
                    return HandleSetReminder(nlpResult);
                ...
                case Intent.StartQuiz:
                    ...
                    return "QUIZ_START"; // Sentinel value — MainWindow opens the quiz panel on seeing this.

                case Intent.ShowActivityLog:
                    return _activityLog.GetFormattedSummary(10);

                case Intent.ShowFullActivityLog:
                    return _activityLog.GetFullFormattedSummary();
        ...
        private string HandleAddTask(NlpResult nlpResult)
        {
            string title = nlpResult.ExtractedTitle;
            string description = $"{title} to help keep your accounts and data secure.";

            DateTime? reminderDate = nlpResult.ReminderDays.HasValue
                ? DateTime.Now.AddDays(nlpResult.ReminderDays.Value).Date
                : (DateTime?)null;

            int newId = _taskManager.AddTask(title, description, reminderDate);

            if (reminderDate.HasValue)
            {
                _activityLog.Log($"Task added: '{title}' (Reminder set for {nlpResult.ReminderDays} day(s) from now).");
                return $"Task added: '{title}'. I've also set a reminder for {nlpResult.ReminderDays} day(s) from now. (Task #{newId})";
            }

            _activityLog.Log($"Task added: '{title}' (no reminder set).");
            return $"Task added: '{title}'. Would you like a reminder? You can say something like \"remind me in 3 days\". (Task #{newId})";
        }

        private string HandleSetReminder(NlpResult nlpResult)
        {
            CyberTask target = _taskManager.GetMostRecentTask();

            if (target == null)
                return "You don't have any tasks yet — add one first by saying something like \"add a task to enable 2FA\".";

            DateTime reminderDate = nlpResult.ReminderDate
                ?? (nlpResult.ReminderDays.HasValue ? DateTime.Now.AddDays(nlpResult.ReminderDays.Value).Date : DateTime.Now.AddDays(1).Date);

            _taskManager.SetReminder(target.Id, reminderDate);

            string whenText = nlpResult.ReminderDays.HasValue
                ? $"{nlpResult.ReminderDays} day(s) from now"
                : reminderDate.ToString("dd MMM yyyy");

            _activityLog.Log($"Reminder set: '{target.Title}' on {reminderDate:dd MMM yyyy}.");
            return $"Got it! I'll remind you about '{target.Title}' in {whenText}.";
        }
        ...
        private string HandleViewTasks()
        {
            ...
            foreach (var t in tasks)
            {
                ...
                sb.AppendLine($"#{t.Id} [{status}] {t.Title}{reminder}");
            }

            return sb.ToString().TrimEnd();
        }

        private string HandleCompleteTask(NlpResult nlpResult)
        {
            int? id = nlpResult.ReferencedTaskId;

            if (!id.HasValue)
            {
                var recent = _taskManager.GetMostRecentTask();
                if (recent == null)
                    return "You don't have any tasks to complete yet.";
                id = recent.Id;
            }

            bool success = _taskManager.MarkCompleted(id.Value);

            if (!success)
                return $"I couldn't find a task with #{id.Value}. Try \"show my tasks\" to see valid task numbers.";

            _activityLog.Log($"Task #{id.Value} marked as completed.");
            return $"Nice work! Task #{id.Value} has been marked as completed.";
        }

        private string HandleDeleteTask(NlpResult nlpResult)
        {
            int? id = nlpResult.ReferencedTaskId;

            if (!id.HasValue)
                return "Please tell me which task number to delete, e.g. \"delete task 2\". Try \"show my tasks\" to see the numbers.";

            bool success = _taskManager.DeleteTask(id.Value);

            if (!success)
                return $"I couldn't find a task with #{id.Value}.";
        ...
        {
            IsQuizActive = false;
            string feedback = _quizEngine.GetFinalFeedback();
            _activityLog.Log($"Quiz completed - scored {_quizEngine.Score}/{_quizEngine.TotalQuestions}.");
            return feedback;
        }
    }
}
